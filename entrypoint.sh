...
log() { echo "[windrose] $*"; }
quote() { printf '%q' "$1"; }
run_as_steam() { su -s /bin/bash steam -c "$*"; }
...
update_server() {
    ...
    log "Updating or validating server files via SteamCMD (AppID $APPID)"
...
find_server_exe() {
    find "$SERVERDIR" -iname 'WindroseServer-Win64-Shipping.exe' 2>/dev/null | head -n 1 || true
}
...
start_server() {
    ...
    log "Executable: $exe"
    log "Bind: $MULTIHOME  Port: $PORT  QueryPort: $QUERYPORT"
...
if [ -z "$SERVER_EXE" ]; then
    log "ERROR: Windrose server executable not found under $SERVERDIR"
